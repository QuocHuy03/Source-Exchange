...
if ($_POST['type'] == 'PlusMoney' && $getUser['level']=="admin") {
    ...
    if($create)
    {
        $LQH->cong("users", "money", $money, " `username` = '".$user."' ");
...
if ($_POST['type'] == 'Deduction' && $getUser['level']=="admin") {
    ...
    if($create)
    {
        $LQH->tru("users", "money", $money, " `username` = '".$user."' ");
