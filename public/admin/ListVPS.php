...
if (isset($_POST['ThemVPS'])) {
    $isInsert = $LQH->insert("vps", [
        'name_vps'       => check_string($_POST['name_vps']),
        'price_vps'      => check_string($_POST['price_vps']),
        'server_vps'     => check_string($_POST['server_vps']),
        'gb_vps'         => check_string($_POST['gb_vps']),
        'code'               => check_string(slugHuyDev($_POST['name_vps']))
    ]);
    if ($isInsert) {
                ...
                <div class="card-body">
                    <div class="table-responsive p-0">
                        <table id="datatable1" class="table table-bordered table-striped table-hover">
                            ...
                            <tbody>
                                <?php $i = 0;
                                foreach ($LQH->get_list("SELECT * FROM `vps` ORDER BY `id` DESC") as $row) { ?>
                                    <tr>
                                        <td><?= $i++ ?></td>
                                        <td><?= $row['name_vps']; ?></td>
                                        <td><?= format_cash($row['price_vps']) ?></td>
                                        <td><?= $row['server_vps']; ?></td>
                                        <td><?= $row['gb_vps']; ?></td>
                                        <td>
                                            <a aria-label="" href="<?= BASE_URL('/admin/ListVPS/Edit/') ?><?= $row['id'] ?>" style="color:white;" class="btn btn-info btn-sm btn-icon-left m-b-10" type="button">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">THÊM GÓI CLOUD VPS</h4>
            ...
            <div class="modal-body">
                <form role="form" action="" method="post">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="name_vps" placeholder="Nhập tên gói" required="">
                    </div>
                    <div class="form-group">
                        <label for="exampleInputEmail1">GIÁ GÓI</label>
                        <input type="text" class="form-control" name="price_vps" placeholder="Nhập giá gói" required="">
                    </div>
                    <div class="form-group">
                        <label for="exampleInputEmail1">SERVER</label>
                        <input type="text" class="form-control" name="server_vps" placeholder="Nhập server" required="">
                    </div>
                    <div class="form-group">
                        <label for="exampleInputEmail1">DUNG LƯỢNG</label>
                        <input type="text" class="form-control" name="gb_vps" placeholder="Nhập GB" required="">
                    </div>
            </div>
            <div class="modal-footer justify-content-between">
                <button type="button" class="btn btn-default" data-dismiss="modal">ĐÓNG</button>
                <button type="submit" name="ThemVPS" class="btn btn-primary">THÊM</button>
            </div>
            </form>
        </div>
        <!-- /.modal-content -->
...
<script>
    function RemoveRow(id) {
        cuteAlert({
            type: "question",
            title: "Xác Nhận Xóa Cloud VPS",
            message: "Bạn có chắc chắn muốn xóa Cloud VPS ID " + id + " không ?",
            confirmText: "Đồng Ý",
            cancelText: "Hủy"
        }).then((e) => {
            if (e) {
                $.ajax({
                    url: "<?= BASE_URL('') ?>/assets/ajaxs/admin/removeVPS.php",
                    method: "POST",
                    dataType: "JSON",
                    data: {
                        id: id
