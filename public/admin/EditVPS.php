...
<?php
if (isset($_GET['id'])) {
    $row = $LQH->get_row(" SELECT * FROM `vps` WHERE `id` = '" . check_string($_GET['id']) . "'  ");
    if (!$row) {
    }
} else {
}
if (isset($_POST['Save'])) {
    $isInsert = $LQH->update("vps", array(
                    ...
                    <form action="" method="POST" enctype="multipart/form-data">
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="name_vps" value="<?= $row['name_vps'] ?>" placeholder="Nhập tên gói" required>
                            </div>
                            <div class="form-group">
                                <label for="exampleInputEmail1">GIÁ GÓI</label>
                                <input type="text" class="form-control" name="price_vps" value="<?= $row['price_vps'] ?>" placeholder="Nhập giá gói" required>
                            </div>
                            <div class="form-group">
                                <label for="exampleInputEmail1">SERVER</label>
                                <input type="text" class="form-control" name="server_vps" value="<?= $row['server_vps'] ?>" placeholder="Nhập server" required>
                            </div>
                            <div class="form-group">
                                <label for="exampleInputEmail1">DUNG LƯỢNG</label>
                                <input type="text" class="form-control" name="gb_vps" value="<?= $row['gb_vps'] ?>" placeholder="Nhập GB" required>
                            </div>
