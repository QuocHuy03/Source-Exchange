...
<?php
if (isset($_GET['id'])) {
    $row = $LQH->get_row(" SELECT * FROM `category` WHERE `id_category` = '" . check_string($_GET['id']) . "'  ");
    if (!$row) {
    }
} else {
}
if (isset($_POST['Save'])) {
    $isInsert = $LQH->update("category", array(
                    ...
                    <form action="" method="POST" enctype="multipart/form-data">
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="nameCategory" value="<?= $row['nameCategory'] ?>" placeholder="Nhập tên Danh Mục" required>
                            </div>
