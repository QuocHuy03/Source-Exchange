                    ...
                    <div class="card card-primary card-outline">
                        ...
                        <form action="" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="card-footer clearfix">
                                <button name="ThemNganHang" class="btn btn-info btn-icon-left m-b-10" type="submit"><i class="fas fa-plus mr-1"></i>Thêm Ngay</button>
                            </div>
                        </form>
                    </div>
                </section>
                        ...
                        <div class="card-body">
                            <div class="table-responsive p-0">
                                <table id="datatable1" class="table table-bordered table-striped table-hover">
                                    ...
                                    <tbody>
                                        <?php $i = 0;
                                        foreach ($LQH->get_list("SELECT * FROM `bank` ORDER BY `id` DESC") as $row) { ?>
                                            <tr>
                                                <td><?= $i++ ?></td>
                                                <td>
                                                    <?= $row['short_name'] ?>
...
<script>
    function RemoveRow(id) {
        cuteAlert({
            type: "question",
            title: "Xác Nhận Ngân Hàng",
            message: "Bạn có chắc chắn muốn Ngân Hàng " + id + " không ?",
            confirmText: "Đồng Ý",
            cancelText: "Hủy"
        }).then((e) => {
            if (e) {
                $.ajax({
                    url: "<?= BASE_URL('') ?>/assets/ajaxs/admin/removeBank.php",
                    method: "POST",
                    dataType: "JSON",
                    data: {
                        id: id
