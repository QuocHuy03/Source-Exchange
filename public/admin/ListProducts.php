...
CheckLogin();
CheckAdmin();
if (isset($_POST['ThemProducts'])) {
    $isInsert = $LQH->insert("products", [
                        ...
                        <table id="datatable1" class="table table-bordered table-striped table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th>HÌNH ẢNH</th>
                                    <th>PRICE</th>
                                    <th style="width:250px">LINK CODE</th>
                                    <th>DESC</th>
                                    <th>THAO TÁC</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php $i = 0;
                                foreach ($LQH->get_list("SELECT * FROM `products` ORDER BY `id_products` DESC") as $row) { ?>
                                    <tr>
                                        <td><?= $i++ ?></td>
                                        <td><?= $row['nameProducts']; ?></td>
                                        <td><img src="<?= $row['imgProducts']; ?>" alt="" width="200"></td>
                                        <td><?= format_cash($row['priceProducts']) ?></td>
                                        <td><?= $row['linkProducts']; ?></td>
                                        <td><?= $row['describeProducts']; ?></td>
                                        <td>
                                            <a aria-label="" href="<?= BASE_URL('/admin/ListProduct/Edit/') ?><?= $row['id_products'] ?>" style="color:white;" class="btn btn-info btn-sm btn-icon-left m-b-10" type="button">
                                                <i class="fas fa-edit mr-1"></i><span class="">Edit</span>
                                            </a>
                                            <button style="color:white;" onclick="RemoveRow(<?= $row['id_products'] ?>)" class="btn btn-danger btn-sm btn-icon-left m-b-10" type="button">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form role="form" action="" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="imgProducts" placeholder="Nhập Link Địa Chỉ Ảnh" required="">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="describeProducts" class="form-control" placeholder="Nhập Mô Tả Sản Phẩm">
                    </div>
                    <div class="form-group">
            ...
            <div class="modal-footer justify-content-between">
                ...
                <button type="submit" name="ThemProducts" class="btn btn-primary">THÊM</button>
            </div>
            </form>
        </div>
        <!-- /.modal-content -->
...
<script>
    function RemoveRow(id) {
        cuteAlert({
            ...
            if (e) {
                $.ajax({
                    url: "<?= BASE_URL('') ?>/assets/ajaxs/admin/removeProducts.php",
                    method: "POST",
                    dataType: "JSON",
                    data: {
                        id: id
