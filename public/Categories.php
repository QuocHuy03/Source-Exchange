...
$title = 'CATEGORYS | ' . $LQH->site('title');
                        ...
                        <div class="grid grid-cols-12 gap-6">
                            ...
                            <div class="col-span-12 mt-8">
                                ...
                                <div class="intro-y flex items-center">
                                    <div class="quochuy">
                                        <h2 class="font-bold text-2xl mr-auto">
                                            <?php foreach ($LQH->get_list("SELECT * FROM category WHERE codeCategory =  '" . $_GET['code'] . "'") as $row) { ?>
                                                <?= $row['nameCategory'] ?>
                                            <?php } ?>
                                        </h2>
                                    </div>
                                ...
                                <div class="grid grid-cols-12 gap-6 mt-5">
                                    <!-- Product Huy Nhé -->
                                    <?php foreach ($LQH->get_list("SELECT * FROM products,category WHERE products.typeProducts = category.codeCategory AND products.typeProducts = '" . check_string($_GET['code']) . "' ORDER BY products.code DESC") as $row) { ?>
                                        <div class="intro-y col-span-12 md:col-span-6 lg:col-span-4 xl:col-span-3">
                                            <div class="box">
                                                <div class="p-5">
                                                    <div class="h-40 2xl:h-56 image-fit rounded-md overflow-hidden before:block before:absolute before:w-full before:h-full before:top-0 before:left-0 before:z-10 before:bg-gradient-to-t before:from-black before:to-black/10">
                                                        <img alt="Mẫu Shop Game" class="rounded-md" src="<?= $row['imgProducts'] ?>">
                                                        <span class="absolute top-0 bg-pending/80 text-white text-xs m-5 px-2 py-1 rounded z-10"> Mới </span>
                                                        <div class="absolute bottom-0 text-white px-5 pb-6 z-10">
                                                            <a href="<?= BASE_URL('/products/') ?><?= $row['code'] ?>" class="block font-medium text-base"><?= $row['nameProducts'] ?></a>
                                                    ...
                                                    <div class="text-slate-600 dark:text-slate-500 mt-5">
                                                        <div class="flex items-center">
                                                            ...
                                                            </svg> Price : <?= format_cash($row['priceProducts']) ?>
                                                        </div>
                                                ...
                                                <div class="flex justify-center lg:justify-end items-center p-5 border-t border-slate-200/60 dark:border-darkmode-400">
                                                    <a class="flex items-center text-success mr-auto" href="<?= BASE_URL('/products/') ?><?= $row['code'] ?>">
                                                    ...
                                                    <a href="javascript:;" onclick="addCart(<?= $row['id_products'] ?>)" class="btn btn-primary" role="button">

                                                        <?php if ($row['priceProducts'] == '0') { ?>
                                                            Miễn Phí
                                                        <?php } else { ?>
                                                            <?= format_cash($row['priceProducts']) ?> Coin
                                                        <?php } ?></a>
                                                </div>
                                            </div>
                                        </div>
